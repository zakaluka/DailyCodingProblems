...
//!
//! The full problem statement can be found on the [Advent of Code 2019][1]
...
//! found or generated from the [GitHub repository][2]._
//!
//! ## Problem 1a
//!
...
//! # Solution
//!
//! The solutions to both problems are quite simple.  The full source code is
//! available at the [above link][2].
//!
//! However, I will highlight a realization I had - and repeatedly have, so
//! maybe I need to do a better job of learning my lesson the first time!
//!
...
//! ```
//!
//! While this code is simple, it contains an insidious bug.  It has to do with
//! an invariant of `u64`, specifically around subtraction.
//!
//! The problem is that when a subtraction occurs with two `u64` values, it can
//! result in an overflow if the second number is larger than the first (e.g.
//! `1u64 - 2u64`).  This causes a panic.
//!
//! Now, when I was testing Problem 1a, I only used the samples provided in the
...
//!
//! There is a very easy way I could have avoided this problem - by using
//! property-based testing.  I have used the [proptest crate][3] in the past,
...
//! of Code problem.
//!
//! When I hit the issue with Problem 1b, I re-wrote the same algorithm as
//! follows:
//!
//! ```text
//! pub fn calculate_fuel(mass: u64) -> u64 {
...
//! ```
//!
//! The `cf_helper` function is unique to Problem 1b.  However,
//! `calculate_single_fuel` is the new version of the previous `calculate_fuel`.
//! Even though the function is small, it is twice the size of the original with
//! a new conditional check.
//!
//! This check prevents the aforementioned issue with the code.  After I found
...
//! property-based testing to Problem 1b.
//!
//! As you can imagine, `proptest` immediately identified the obvious mirror to
//! the problem, i.e. calculating a required fuel that is so large that it
//! overflows the maximum value of `u64`.  This issue was found when performing
//! property-based testing on `calculate_fuels`, whose source code is below.
//!
//! ```text
...
//! ```
//!
//! And quite helpfully, `proptest` identified a minimal test case to cause the
//! overflow.
//!
//! ```text
//! t = [
...
//! ```
//!
//! However, I did not fix this problem programmatically and for good reason (I
...
//! program will panic.
//!
//! What does this mean from a "real-world" perspective?  If this was a function
...
//! with its own invariants, returning a `Result`, etc.). However, if it is not,
...
//! to understand how it should be handled.  This [article][4] from Scott
...
//! re-reviewed and re-discussed thoroughly.
//!
//! In this instance, I have left the code as-is and have not addressed the
//! issue with Problem 1b's `calculate_fuels` method.
//!
//! # Conclusion
//!
//! I seem to always be re-learning a lesson when it comes to property-based
...
//! improve and be of higher quality when it is delivered.
//!
//! As for the "bug" that has been left in the program, I do believe that
...
pub mod aoc2019_1b {
  ...
  /// Assumptions:
  ///
  /// 1. The sum of the fuel (including fuel needed for the additional fuel)
  /// will not be greater than `std::u64::max_value()`.
  pub fn calculate_fuels(mass: Vec<u64>) -> u64 {
    mass
      .iter()
      .map(|e| calculate_fuel(*e))
