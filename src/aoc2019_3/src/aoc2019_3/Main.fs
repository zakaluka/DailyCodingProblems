...
module Say =
  ...
  let colorizeIn color str =
    ...
    Console.ForegroundColor <-
      (Enum.Parse(typedefof<ConsoleColor>,color) :?> ConsoleColor)
    printfn "%s" str
...
module Main =
  ...
  let main(argv: string array) =
    let parser = ArgumentParser.Create<CLIArguments>(programName = "aoc2019_3")
    let results = parser.Parse(argv)
    if results.Contains Version then
      AssemblyInfo.printVersion()
    else if results.Contains Info then
      AssemblyInfo.printInfo()
    else if results.Contains Hello then
