module aoc2019_3.bolero.Client.State
...
let update msg model =
  ...
  | CreatePaths ->
      let p1 =
        Path.tryParse model.UI.Input1
        |> Result.defaultValue Path.empty

      let p2 =
        Path.tryParse model.UI.Input2
        |> Result.defaultValue Path.empty
  ...
  | CreateWires ->
      let w1 =
        Wire.tryParse model.PathOne
        |> Result.defaultValue Wire.empty

      let w2 =
        Wire.tryParse model.PathTwo
        |> Result.defaultValue Wire.empty
  ...
  | GetAllIntersectionPoints ->
      ...
      | Error (_) ->
          { model with IntersectionPoints = [] }, Cmd.ofMsg UiSetError
  | UiChangeInput1 t ->
      { model with
          UI = { model.UI with Input1 = t } },
      Cmd.none
  | UiChangeInput2 t ->
      { model with
          UI = { model.UI with Input2 = t } },
      Cmd.none
  | UiSetError ->
      { model with
          UI = { model.UI with Error = true } },
      Cmd.none
  | UiClearError ->
      { model with
          UI = { model.UI with Error = false } },
      Cmd.none
