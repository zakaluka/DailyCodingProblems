module aoc2019_3.bolero.Client.Types
...
module Path =
  ...
  module Distance =
    ...
    let tryParse (s: string) =
      ...
      if success
      then if x > 0 then create x else Error "Zero distance not allowed"
      else Error(sprintf "Unable to parse as Int32: %s" s)
  ...
  module PathEntry =
    ...
    let tryParse (s: string) =
      let validate (s: string) =
        if s.Length < 2
        then Error(sprintf "String not of type '[UDLR][0-9]+': %s" s)
        else Ok s
...
module Wire =
  ...
  type Point =
    { x: int
      y: int }
    override p.ToString() = sprintf "(%d,%d)" p.x p.y
  ...
  module Point =
    ...
    let min p q =
      if manhattanDistance p MIN <= manhattanDistance q MIN
      then p
      else q
  ...
  module WireSection =
    ...
    let length ws = (abs(ws.End.x - ws.Start.x))+(abs(ws.End.y - ws.Start.y))
    ...
    let intersectOneHOneV ws1 ws2 =
      let horizL, vertL =
        if ws1.Orientation = H then ws1, ws2 else ws2, ws1
      // Both lines have the expected orientation
      if horizL.Orientation <> H || vertL.Orientation <> V then
        None
      // `x` values don't intersect
      elif vertL.Start.x < horizL.Start.x
           || vertL.Start.x > horizL.End.x then
        None
      // `y` values don't intersect
      elif horizL.Start.y < vertL.Start.y
           || horizL.Start.y > vertL.End.y then
        None
      // Find point of overlap
      else
        Some
  ...
  module Wire =
    ...
    let intersectionPoints w1 w2 =
      let intersectionWtoWS (Wire (w)) ws wsLengthToDate =
        result {
          let! pointsOfIntersection =
            w
            |> List.mapi (fun i wWs ->
                 let ws2LengthToDate =
                  [0..i-1]|> List.sumBy (fun j -> WireSection.length w.[j])
                 match WireSection.intersection ws wWs with
                 | Ok (x) ->
                    let ws2Update =
                      ws2LengthToDate +
                        (match x with
                        |Some (p) ->(abs(p.x-wWs.Start.x)) + (abs(p.y-wWs.Start.y))
                        | None -> 0)
                    Ok(ws2Update, x)
                 | Error (e) -> Error(e))
            |> List.sequenceResultA
          ...
          return pointsOfIntersection
                 ...
                 |> List.map (fun (ws2LTD, po) -> ws2LTD, Option.get po)
                 |> List.map (fun (ws2LTD, p) ->
                      { pt = p
                        distanceFromCenter =
                          Point.manhattanDistance p Point.CENTRAL_PORT
                        distanceBySections = ws2LTD + wsLengthToDate+ (abs(p.x - ws.Start.x))+(abs(p.y-ws.Start.y)) })
        }
      ...
      result {
        ...
        let! allIntersections =
          wire1
          |> List.mapi (fun i ws ->
                          let wsLengthToDate = [0..i-1]|>List.sumBy(fun j -> WireSection.length wire1.[j])
                          intersectionWtoWS w2 ws wsLengthToDate )
          |> List.sequenceResultA

        return allIntersections |> List.collect id
      }
...
type Model =
  { UI: ModelUi
    ...
    WireOne: Wire.Wire
    WireTwo: Wire.Wire
    IntersectionPoints: Wire.IntersectionPoint list }
